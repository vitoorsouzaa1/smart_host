  ...
  return (
    <div className="mb-16">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-4">Discover our most popular vacation rentals</p>
      </div>
      <div 
        ref={carouselRef} 
      ...
      >
        ...
        {properties.map((property) => (
          <div key={property.id} className="min-w-[280px] md:min-w-[calc(50%-12px)] lg:min-w-[calc(25%-18px)] snap-start">
            <PropertyCard
              id={property.id}
      ...
      <div className="flex justify-center mt-6 gap-1">
        {Array.from({ length: Math.ceil(properties.length / visibleCards) }).map((_, index) => {
          const isActive = 
            index === Math.floor(scrollPosition / (carouselRef.current?.clientWidth || 1) * Math.ceil(properties.length / visibleCards) / properties.length);
          return (
            <button 
              key={index} 
              className={`w-2 h-2 rounded-full ${isActive ? 'bg-blue-600' : 'bg-gray-300'}`}
              onClick={() => {
                if (carouselRef.current) {
                  const newPosition = (carouselRef.current.scrollWidth / Math.ceil(properties.length / visibleCards)) * index;
                  carouselRef.current.scrollTo({ left: newPosition, behavior: 'smooth' });
                  setScrollPosition(newPosition);
                }
              }}
